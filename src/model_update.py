...
# preprocess data -> split dataset -> update model -> save -> get report
##########################################################################################
...
def update(**kwargs) -> Tuple[CatBoostRegressor, float]:
    '''
    Parameters
...
def save(**kwargs) -> None:
    ...
    Returns
    '''
    model, score = kwargs['task_instance'].xcom_pull(task_ids='update')
    path = kwargs['path']
...
with DAG(
    ...
    data = os.path.join(os.path.dirname(__file__), 'input', 'update', 'data.csv')
    ...
    model_path = os.path.join(os.path.dirname(__file__), 'input', 'update', model_name)
    ...
    # Task 4: save
    path = os.path.join(os.path.dirname(__file__), 'output', 'update')
    model_name = 'model.pkl'
